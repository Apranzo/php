...
$prod_filter = '';
function get_products_f($filter) {
	$fd = init_fd_start()['products'];
	$prods = [];
	$GLOBALS['prod_filter'] = $filter;
	while(!feof($fd)) {
		$prods[] = json_decode(fgets($fd), true);
	}
	if ($filter) {
		$prods = array_filter($prods, static function ($k) {
			if (strpos($k['name'], $GLOBALS['prod_filter']) ||
				strpos($k['desc'], $GLOBALS['prod_filter'])
			) {
				return true;
			}
			if (!$k['tag'])
			...
			foreach ($k['tag'] as $tag)
				if (strpos($tag, $GLOBALS['prod_filter']))
					return true;
			return false;
